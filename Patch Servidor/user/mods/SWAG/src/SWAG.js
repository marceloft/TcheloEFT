...
const partisan_json_1 = __importDefault(require("../config/bosses/partisan.json"));
// Spawn Configs
...
class SWAG {
    ...
    preSptLoad(container) {
        ...
        staticRouterModService.registerStaticRouter(`${modName}/client/match/local/end`, [{
   ...
   */
    ...
    static configureMaps() {
        ...
        ClassDef_1.validMaps.forEach((globalmap) => {
    ...
    static spawnBosses(boss, globalmap) {
        if (bossConfig_json_1.default.TotalBossesPerMap[ClassDef_1.reverseMapNames[globalmap]] == 0) {
            if (config_json_1.default.DebugOutput)
            ...
            const wave = SWAG.configureBossWave(boss, globalmap);
    ...
    static configureBossWave(boss, globalmap) {
        ...
        const bossName = ClassDef_1.roleCase[boss.BossName.toLowerCase()] || boss.BossName;
        ...
        const difficultyKey = boss.BossDifficult || config_json_1.default.BossDifficulty.toLowerCase();
        const difficulty = difficultyKey === "asonline" ? getRandomDifficulty() : ClassDef_1.diffProper[difficultyKey];
        const escortDifficultyKey = boss.BossEscortDifficult || config_json_1.default.BossEscortDifficulty.toLowerCase();
        const escort_difficulty = escortDifficultyKey === "asonline" ? getRandomDifficulty() : ClassDef_1.diffProper[escortDifficultyKey];
        ...
        if (spawnChance != 0 && config_json_1.default.DebugOutput) {
    ...
    static disableSpawnSystems(container) {
            ...
            if (hostilitySettings) {
                for (let i = hostilitySettings.length - 1; i >= 0; i--) {
                    ...
                    if (setting.BotRole == "pmcUSEC" || setting.BotRole == "pmcBEAR") {
                        ...
                        const newSetting = { ...pmcHostilitySettings, BotRole: setting.BotRole };
